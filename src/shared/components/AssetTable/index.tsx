        ...
        <TableBody color={'white'}>
          {data.map(data => (
            <TableRow key={data.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
              ...
              {data.avgBuyPrice ? <TableCell align='left'>{data.avgBuyPrice.toFixed(2)}</TableCell> : <></>}
              {data.totalQuantity ? <TableCell align='left'>{data.totalQuantity.toFixed(2)}</TableCell> : <></>}
